...
final class MockExplorationClientTests: XCTestCase {
    ...
    func testEquialency() async throws {
        ...
        XCTAssertTrue(
            mockClient.mapsAreEquivalent(map1: mockClient.correctMap!, map2: mockClient.correctMap!)
        )
    }
    ...
    func testEquialency2() async throws {
        let mockClient = MockExplorationClient()
        XCTAssertTrue(
            mockClient.mapsAreEquivalent(
                map1: MockExplorationClient.generateThreeRooms(offset: 0),
